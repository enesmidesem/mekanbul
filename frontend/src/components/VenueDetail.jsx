...
const VenueDetail = () => {
  ...
  .then((response) => {
    // Veriyi dönüştürerek kaydet
    const formattedData = {
      ...response.data,
      hours: (response.data.hours || []).map(hour => ({
        days: hour.day,  // day -> days olarak eşitle
        open: hour.isClosed ? "Kapalı" : hour.open,
        close: hour.isClosed ? "Kapalı" : hour.close
      }))
    };
            ...
            <div className="col-xs-12 ">
              <div className="panel panel-primary">
                <div className="panel-heading ">
                  ...
                  */}
                  {/* Giriş yapmışsa Yorum Ekle butonu, yapmamışsa Giriş Yap butonu */}
                  {isLoggedIn ? (
                    <NavLink
                    ...
                    <NavLink
                      className="btn btn-default pull-right" 
                      to="/login"
                      // Kullanıcı giriş yaptıktan sonra bu sayfaya geri dönsün diye path veriyoruz (Opsiyonel)
                      state={{ from: `/venue/${id}` }} 
                    >
